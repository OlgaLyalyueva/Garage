...
class TestCarIssuesArchivedView(TestCase):
    ...
    def test_logged_in_user_receives_archived_issues(self):
        ...
        response = c.get('/issues/archived/')
        self.assertEqual(len(response.context['page_obj_issues'].object_list), 3)
    ...
    def test_logged_in_user_receives_all_issues_data(self):
        ...
        response = c.get('/issues/archived/')
        issues = response.context['page_obj_issues']
        self.assertEqual(len(issues.object_list), 3)
        self.assertIsNone(response.context['message'])
        self.assertEqual(issues.object_list[0].name, first_issue.name)
        self.assertEqual(issues.object_list[0].description, first_issue.description)
        self.assertEqual(issues.object_list[0].date, first_issue.date)
        self.assertTrue(issues.object_list[0].close)
        self.assertTrue(issues.object_list[0].archive)
        self.assertEqual(issues.object_list[0].car, first_issue.car)

        self.assertEqual(issues.object_list[1].name, second_issue.name)
        self.assertEqual(issues.object_list[1].description, second_issue.description)
        self.assertEqual(issues.object_list[1].date, second_issue.date)
        self.assertTrue(issues.object_list[1].close)
        self.assertTrue(issues.object_list[1].archive)
        self.assertEqual(issues.object_list[1].car, second_issue.car)

        self.assertEqual(issues.object_list[2].name, third_issue.name)
        self.assertEqual(issues.object_list[2].description, third_issue.description)
        self.assertEqual(issues.object_list[2].date, third_issue.date)
        self.assertFalse(issues.object_list[2].close)
        self.assertTrue(issues.object_list[2].archive)
        self.assertEqual(issues.object_list[2].car, third_issue.car)
